...
class FileRepositoryTest {
    ...
    void findByIdAndIsRemovedFalse_File이_삭제되지_않았으면_정상반환() {
        ...
        File file1 = p.addNewFile("example1.txt", "post/1/example1-stored.txt");
        File file2 = p.addNewFile("example2.txt", "post/1/example2-stored.txt");
    ...
    void findAllByPostIdAndIsRemovedFalse_삭제되지_않은_모든_File_반환() {
        ...
        for (int i = 1; i <= 6; i++) {
            File file = (i % 2 == 0) ? p1.addNewFile("file" + i + ".txt", "post/1/file" + i + "_stored.txt") : p2.addNewFile("file" + i + ".txt", "post/1/file" + i + "_stored.txt");
